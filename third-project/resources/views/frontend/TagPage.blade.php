    ...
    <section class="page-header">
        <div class="container-xl">
            <div class="text-center">
                <h1 class="mt-0 mb-2">{{ $tag_name }}</h1>
                        ...
                        <div class="col-sm-4">
                            ...
                            <div class="post post-grid rounded bordered">
                                <div class="thumb top-rounded">
                                    <a href="{{ route('category.page', $tag->relation_to_blog->category_id) }}" class="category-badge position-absolute">
                                        {{ $tag->relation_to_blog->relation_to_category->category_name }}
                                    </a>
                                    @if ($tag->relation_to_blog->blog_icon)
                                    <span class="post-format">
                                        <i class="{{ $tag->relation_to_blog->blog_icon }}"></i>
                                    </span>
                                    @else
                                    <span class="post-format">
                                        <i class="icon-user"></i>
                                    ...
                                    @endif
                                    <a href="{{ route('blog.details', $tag->relation_to_blog->id) }}">
                                        <div class="inner">
                                            <img src="{{ asset('upload/blog_photos') }}/{{ $tag->relation_to_blog->blog_photo }}" alt="post-title" />
                                        </div>
                                    </a>
                                </div>
                                <div class="details">
                                    <ul class="meta list-inline mb-0">
                                        <li class="list-inline-item">
                                            <a href="{{ route('personal.page', $tag->relation_to_blog->blogger_id) }}">
                                                @if ($tag->relation_to_blog->relation_to_user->profile_photo)
                                                <img width="40" src="{{ asset('upload/profile_photos') }}/{{ $tag->relation_to_blog->relation_to_user->profile_photo }}" class="author" alt="author"/>
                                                @else
                                                <img width="40" src="{{ asset('upload/profile_photos/default_profile_photo.jpg') }}">
                                                @endif
                                                {{ $tag->relation_to_blog->relation_to_user->name }}
                                            </a>
                                        </li>
                                        <li class="list-inline-item">{{ $tag->relation_to_blog->created_at->format('d M Y') }}</li>
                                    </ul>
                                    <h5 class="post-title mb-3 mt-3">
                                        <a href="{{ route('blog.details', $tag->relation_to_blog->id) }}">
                                        {{ $tag->relation_to_blog->blog_title }}</a></h5>
                                    <p class="excerpt mb-0">{{ $tag->relation_to_blog->blog_intro }}</p>
                                </div>
                                <div class="post-bottom clearfix d-flex align-items-center">
                                    ...
                                    <div class="more-button float-end">
                                        <a href="{{ route('blog.details', $tag->relation_to_blog->id) }}"><span class="icon-options"></span></a>
                                    </div>
                                </div>
                            </div>
                        </div>
